...
class Game {
  ...
  addToTable() {
    historiqTableElement.style.display = 'grid'
...
if (historiqTable.length === 0) {
  historiqTableElement.style.display = 'none'
}
historiqTable.forEach((game) => {
  //filling games table
  const temp = document.createElement('div')
  temp.innerHTML = `
        <p>${game.id}</p>
...
function emptyMap() {
  Array.from(mapElement.children).forEach((element) => {
    ...
    element.style.backgroundColor = `#544a5e`
  })
  for (let row = 0; row < mapMatrice.length; row++) {
    for (let column = 0; column < mapMatrice[row].length; column++) {
      mapMatrice[row][column] = ''
...
function emptyScoreTable() {
  scoreTableElement.innerHTML = `<div>
        <p>Player name</p>
        <p>Player score</p>
        <p>Player color</p>
    </div>
...
function startGame() {
  ...
  start_restartBtn.style.backgroundColor = '#FFCA3E'
  gameStatus = 'started'
  currentTurn = 0
...
function restartGame() {
  ...
  gameStatus = 'stopped'
  start_restartBtn.style.backgroundColor = '#544a5e'
  start_restartBtn.innerText = 'Commencer'
  currentTurn = 0
  emptyMap()
...
function verifyRow(row) {
  const temp =
    mapMatrice[row][0] === mapMatrice[row][1] &&
    mapMatrice[row][0] === mapMatrice[row][2]
  if (temp) {
...
function verifyColumn(column) {
  const temp =
    mapMatrice[0][column] === mapMatrice[1][column] &&
    mapMatrice[0][column] === mapMatrice[2][column]
  if (temp) {
    //draw horizontal line
    line = createLine()
...
function verifyDiagonal(row, column) {
  if (
    mapMatrice[0][0] != '' &&
    mapMatrice[0][0] === mapMatrice[1][1] &&
    mapMatrice[0][0] === mapMatrice[2][2]
  ) {
    line = createLine()
  ...
  if (
    mapMatrice[1][1] != '' &&
    mapMatrice[0][2] === mapMatrice[1][1] &&
    mapMatrice[0][2] === mapMatrice[2][0]
  ) {
    line = createLine()
